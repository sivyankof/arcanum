...
- `npm test` — юнит-тесты (jest-expo). На 15.08: 635 тестов в 26 сьютах, все зелёные. Правило: новая формула или алгоритм — тест в том же коммите (`docs/testing-strategy.md`). ⚠️ Node несёт полный ICU, а Hermes на телефоне — урезанный, поэтому всё, что считает сам движок (формы числительных, даты, форматы чисел), обычным тестом не проверяется: механизм сносится руками до загрузки модуля — образец `src/lib/__tests__/i18nPlurals.test.ts` (hf-02).
