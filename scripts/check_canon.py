...
#
# ⚠️ Дополнено задачей 37 (15.08) формами, которые первая редакция пропустила: вычитка
# заходов М5–М6 нашла «вы построите», «результаты приближаются», «ответы придут», «сердце
# скажет спасибо» прямо в reviewed-каноне. Правило то же, что в задаче 31: после смысловой
# вычитки проверка обязана выучить форму, мимо которой прошла.
# ⚠️ Широкое будущее время в набор НЕ входит, и это замер, а не осторожность: «you will»
# даёт 23 находки, из которых почти все — норма жанра day_card («by evening you'll notice»),
# «возвращаются» — 12 при одной настоящей. Ловим УЗКИЙ класс «конкретное благо + придёт»:
# дефект не в глаголе будущего времени, а в обещании внешнего блага. Что именно возвращается —
# внутреннее состояние или доход — регулярка не различает, это остаётся человеку.
PREDICTION = {
    "ru": r"сбыл|сбудет|сбывает|(?<!не )обещает|(?<!не )обещают|вас ждёт|вас ожидает|предсказ|"
          ...
          r"(?:результат\w*|признание|помощь|ответы?|деньги|доход|успех|любовь|удача)"
          r"(?:\s+\w+){0,2}\s+(?:придут|придёт|приближ\w+|вернутся|вернётся)|"
          r"вы\s+(?:построите|добьётесь|обретёте|получите|достигнете|найдёте|встретите)|"
          r"скажет\s+спасибо",
    "en": r"will come true|comes true|card promises(?! no)|promises you|promises a |awaits you|"
          ...
          r"(?:results?|help|answers?|recognition|money|income|success|love|luck)"
...
GENDER = [
    ...
    # «делает вас свободным» (задача 37): род читателя, приписанный ему сказуемым.
