...
def check_openings(cards_new, langs, rep, threshold=3):
    ...
    for lang in langs:
        ...
        for fw, n in repeats.most_common():
            ...
            # ⚠️ без вложенных кавычек одного вида: f-строка, содержащая внутри такие же
            # кавычки, — синтаксис PEP 701, доступный только с Python 3.12. На 3.10 файл
            # падает с SyntaxError ещё до разбора аргументов, то есть проверка не выполняется
            # вовсе (нашла Cowork на своей машине; у меня 3.14, и дефект был не виден).
            shown = ', '.join('{}.{}'.format(c['id'], b) for c, b in addrs[:3])
            label = f'«{fw}» ×{n} ({shown})'
