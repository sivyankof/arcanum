...
(async () => {
  ...
  // ⚠️ маркеры «Практика» и «Учёба» — НЕ подписи табов (спека 72, урок 2 ui-verification.md):
  // таб-бар рисует все пять подписей на КАЖДОМ экране разом («Учёба» видна и на «Профиле», и на
  // «Курсе»), поэтому проверка «marker.toUpperCase() ⊂ body» была бы истинна ДО тапа и ничего
  // не проверяла бы. Берём текст, который печатает САМ целевой экран: у «Практики» это заголовок
  // spreads.title «Расклады» (подпись таба переименована в «Практика» спекой 72, заголовок
  // экрана — нет, R7), у «Учёбы» — CTA героя «ОТКРЫТЬ PREMIUM» (сид ниже даёт state=locked).
  for (const [label, marker] of [['Курс', 'МОДУЛЬ'], ['Карты', 'ИЗУЧЕНО'], ['Практика', 'Расклады'], ['Профиль', 'ВАШ ПУТЬ'], ['Учёба', 'ОТКРЫТЬ PREMIUM']]) {
