/* Веб-проверка пейвола без магазина (задача 62, пункт 6б процесса).
   ...
   упало 31 из 58, все пять входов зелёные, ошибок в консоли 0. После правок задач 2–4 — 0 из 58.
   Зелёный с первого раза = ошибка в самой проверке (правило проекта).
   ⚠️ Мутация на слепоту (getOffers возвращает одно предложение без знака валюты) обязана
   ронять по 4 проверки на каждом из шести экранов «скоро» — 24 из 58, а «₽» при этом остаётся
   зелёной. Тариф в мутации обязан быть ГОДОВЫМ: `plan` по умолчанию 'year', и на подсунутом
   `{ id: 'month' }` кнопка берёт запасную надпись (заголовок экрана), проверка «нет "Оформить
   за …"» не срабатывает и мутация роняет только 18 — то есть проверяет меньше, чем кажется.
