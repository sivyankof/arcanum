...
/** Приглушение недоступного/прошедшего: прошедшие дни лунного календаря, заблокированная
 *  карточка/панель лунного расклада вне окна события (спеки 47, 51) — три места держали этот
 *  литерал копией. НЕ тот же токен, что `DIM_OPACITY` в `CardCell.tsx` (0.55, неизученная
 *  карта справочника, спека 46б) — по CSS эталона (`design-reference.html`) это разные значения
 *  (`.mooncal .dim`/`.sp.splocked` дают .45, `.gc.dimmed .im` — .55), объединять их нельзя. */
export const LOCKED_OPACITY = 0.45;
