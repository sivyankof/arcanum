 ...
 *
 *  Живёт в корневом layout, вызывается ровно один раз. Пересчёт происходит в шести точках
 *  из спеки 06б (полный список с обоснованием — logic-spec §8): гидрация стора (первый рендер),
 *  возврат приложения из фона, открытие карты дня, ответ рефлексии, смена настроек и ответ
 *  на системный запрос разрешения — четыре последних приходят сами через подписку на стор.
 */
...
export function usePushScheduler(): void {
  ...
  React.useEffect(() => {
    ...
    // ⚠️ `settings` в зависимостях включает и `pushAsked`, который сам планировщик нигде не
    // читает — на вид лишняя зависимость, и однажды её захотят убрать как «оптимизацию». Нельзя:
    // это единственное, что триггерит пересчёт СРАЗУ после ответа на системный диалог в прелюдии
    // (карта дня, app/daily.tsx) — там флаг специально ставится уже ПОСЛЕ разрешения, чтобы смена `settings`
