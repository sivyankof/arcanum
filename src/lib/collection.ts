 ...
 *  Поле `cards` и порядок по `number` в секциях приложением сегодня не читаются (экран берёт
 *  только open/total) — оставлены как контракт колоды 22+14×4 под тестами и задел для уровней
 *  мастерства (хвост 46); не вырезать как мёртвую поверхность. */
