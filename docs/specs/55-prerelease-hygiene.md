...
Статус: **ВЫПОЛНЕНА 23.08** (согласована Артёмом, отчёт в конце файла; ждёт лайв-проверки 6в). Заведена по решению 23.08 «берём то, что решается
...
Все три пункта закрыты, четыре коммита в `main` (`f2e8877`, `67fcac0`, `d15829b`, `19f6f0c`).

| критерий | факт |
|---|---|
| `expo-doctor` | было 17/18 → стало **18/18** |
| `tsc --noEmit` | чист |
| `npm test` | **1561 тест в 45 сьютах** (было 1551 в 44) |
| прямые вызовы в `app/review.tsx` | нет, стережёт контракт по исходнику |
| иконка пуша | 96×96 RGBA, только белый, доля непрозрачных 25.7 % |
| веб-проверка 6б | `scripts/check_55_web.js` — **7 из 7**, скриншоты `docs/screenshots/55/` |

### Пункт 1 — патчи
`expo` 54.0.35 → 54.0.37, `expo-constants` 18.0.13 → 18.0.14, `expo-file-system` 19.0.23 → 19.0.24,
`jest-expo` 54.0.17 → 54.0.18. `app.json` командой не тронут (проверено диффом).
⚠️ Находка сверх ожидаемого: `expo install` **продублировал `jest-expo`** — новая версия ушла
в `dependencies`, старая осталась в `devDependencies`. Обе записи валидны, ни компилятор, ни тесты
на это не реагируют. Дубль снят руками; урок дописан в `lessons.md` к правилу про `expo install`.

### Пункт 2 — `reviewPrompt`
Формула переехала в `src/lib/review.ts`; экран зовёт одну функцию. Тесты: контракт по корпусу
считает через `reviewPrompt`, синтетический тест расхождения языков — тоже; добавлены контракт
по исходнику (запрещённые вызовы + обязательный `reviewPrompt(`) и парная положительная проверка
«подсказка непустая у всех 78 карт на четырёх языках».
⚠️ Зачем парная: отрицательный контракт «не содержит имени» зелёный и на функции, всегда
возвращающей пустую строку. Мутация «пустой возврат» роняет теперь 5 тестов, до правки — 0.

Мутации (все краснеют поимённо): прямой `maskCardName(` в экране → контракт по исходнику;
`inLang(card.name, lang)` вместо `presentLang` → синтетический тест; пустой возврат → 5 тестов.

### Пункт 3 — иконка пуша
`scripts/gen_notification_icon.py` (PIL, идемпотентен, запуск из корня) рисует силуэт по геометрии
`Emblem.tsx`; `app.json` получил плагин `expo-notifications` с `icon`/`color #caa45a`/
`defaultChannel daily`. Проверка разделена: белизну пикселей держит генератор (перечитывает файл,
код 1 при нарушении), формат и связку «app.json → файл → канал» — `notificationIcon.test.ts`.
⚠️ Замер против «зелёной с первого раза» проверки: 2366 непрозрачных пикселей, из них 1734
полупрозрачных края LANCZOS, и у всех RGB ровно (255,255,255); мутация «золотая заливка» даёт
2352 небелых. Числа записаны в код.
⚠️ Скрипт упал на печати успеха: cp1251-консоль Windows не умеет «×». Потоки настраиваются
на UTF-8 внутри скрипта.

### Веб-проверка 6б
Сценарий открывает `/review` до 12 раз (направление у повторяемых карт случайное), пока не увидит
оба направления. Проверено: рубашка `toCard` показывает чипы и подсказку с маской «···» без имени
карты, лицо `toMeaning` имя показывает, светлая тема цела, консоль чистая.
⚠️ Красный прогон: снятие маски в `reviewPrompt` роняет ровно две проверки маски.
⚠️ Первый прогон покраснел ПО ВИНЕ ПРОВЕРКИ — имена на экране набраны капсом, сценарий искал
строчные. Урок («ошибку ищи в проверке» работает и на красном) — в `lessons.md` §9.

## Сценарий лайв-проверки на iPhone (пункт 6в) — что нажать Артёму

Перед проверкой: `npm install` и перезапуск `npx expo start --tunnel` (менялись `package.json`
и `app.json`).

1. **Тренажёр.** Курс → карточка «Повторение» → пройти 3–4 карточки.
   Ожидание: у карточек-рубашек (те, где сверху чипы со словами, а карты не видно) в подсказке
   стоит «···» вместо названия карты — название не проглядывает нигде до ответа. У карточек
   лицом вверх название показано сразу, маски нет. Поведение обязано быть ровно таким же,
   как до правки, — это рефакторинг.
2. **Смена языка.** Профиль → язык на английский → снова тренажёр: то же самое (маска на месте).
   Вернуть русский.
3. **Всё остальное приложение** — бегло: карта дня, урок, расклад. Проверяем, что патчи Expo
   ничего не сломали.

Чего в этой проверке НЕТ и почему: иконку пуша на телефоне увидеть нельзя — в Expo Go уведомления
показывает сам Expo Go со своей иконкой. Её судья — первая EAS-сборка (этап 6).
