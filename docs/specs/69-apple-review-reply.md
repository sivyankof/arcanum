...
**Ответ пришёл 18.09 20:00 (Developer Support, кейс закрыт):** шаблон без существа — «приложение
отклонено 18 сентября, детали в разделе App Review» плюс инструкция, как включить уведомления.
На вопрос о неотвеченном возражении от 01.09 не ответили. Вывод на будущее: форма App Review Status
сообщает только текущий статус заявки (то же, что даёт `asc_status.js`), спорить по существу через
неё бесполезно — для этого тред App Review и апелляция в App Review Board. Отвечать на письмо не нужно.
