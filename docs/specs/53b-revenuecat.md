...
- [ ] **Google Play заблокирован аккаунтом продавца** (28.08): страница «Подписки» в Play Console
      закрыта замком «нужно настроить аккаунт продавца Google Payments». Беларусь в списке
      стран продавцов есть (USD) — форму платёжного профиля заполняет Артём, только после неё
      создаются подписка `premium` и планы `year`/`month`.
