...
- [~] **Google Play — профиль продавца СОЗДАН 28.08 вечером** (решение Артёма после разбора,
      ...
      (курс целиком / все расклады / тренажёр без лимита), описание, **переводы en-US / es-419 /
      pt-BR** (тексты — `store-listing.md`, раздел «Подписки»; языки добавляются через «Переводы»
      → диалог с поиском по коду, чекбокс — это текст `check_box_outline_blank`, кликать по строке;
      у каждого языка преимущества добавляются заново кнопкой «Добавить преимущество», без
      «Имени» перевода сохранение падает «Не удалось сохранить изменения»). ⚠️ Форма «Set prices»:
...
- [x] **6в Android — ПРОЙДЕНО 28.08 вечером (эмулятор `arcanum_pixel` с Play Store, аккаунт Артёма =
      лицензионный тестер, AAB №2 versionCode 5 из внутреннего трека):** магазин отдал выпуск 5 через
      ~час после публикации (признак — «Есть платный контент» и 47 МБ вместо 45 на витрине); пейвол
      «Год · 99,00 GEL, −56 %» / «Месяц · 19,00 GEL» (валюта аккаунта эмулятора — Грузия), лист Play
      «19,00 GEL/5 min · Тестовая карта, всегда подтверждать» → «Платёж выполнен» (плюс системный
      вопрос «Запрашивать аутентификацию при покупках?» — «Нет») → чип «✦ АКТИВНА», настройки
      «Активна ›», модуль 3 открыт (замок в шапке — маркер платного, как на iOS), «Управлять
      подпиской» → центр подписок Play → отмена с причиной → в приложении «Действует до 28 августа
      2026» через минуту (без RTDN — RevenueCat перепроверяет у Google при обновлении CustomerInfo),
      авиарежим + force-stop + запуск — право на месте, истечение тестового периода (5 мин) → пейвол
      снова с тарифами, повторная покупка → `pm uninstall` → установка из магазина → онбординг →
      «Активна» без нажатия «Восстановить» (`usePremiumSync` на старте). ⚠️ **Дефект и правка**:
      панель писала «Подписка активна» вместо «Месячная подписка» — на Android SDK кладёт в
      `productIdentifier` только id подписки `premium`, а план — в `productPlanIdentifier`
      (`month`/`year`; на iOS — `null`, там всё в `productIdentifier`); `planOf('premium')` → `null`.
      Исправлено в `purchasesMap.ts` (план из пары полей через `:`), тесты с Android-фикстурой,
      красный прогон 1 из 38 на старом коде, коммит `5a16379`. Старая фикстура `premium:month` была
      зелёной на ложной предпосылке о форме ответа SDK. В сборке versionCode 5 дефект остаётся —
      проверить панель на сборке 6. Исходный план (выполнен по нему):
...
## Лайв Android, сборка 6 — 28.08 ~23:10 (эмулятор, внутренний трек)

Проверка правки `5a16379` (`planOf` из `productPlanIdentifier`) на магазинной сборке
`versionCode` 6 (`ca680d94`): на эмуляторе стоял выпуск 5 из Play Store — ссылка внутреннего
трека → «Обновить» → через 95 с `versionCode=6`, `installerPackageName=com.android.vending`,
сплиты прежние. Тестовая подписка с прошлого лайва уже истекла (настройки — «Оформить»),
пейвол показал цены магазина (год 99,00 GEL с бейджем «−56 %», месяц 19,00 GEL — валюта
аккаунта эмулятора), покупка месячного плана тестовой картой («19,00 GEL/5 min») →
панель **«Месячная подписка · Продлится 28 августа 2026»**, кнопка «Управлять подпиской».
На сборке 5 в той же точке панель оставалась без названия тарифа. Кадры —
`docs/screenshots/53b/paywall-android-v6-offers.png` и `paywall-android-v6-month.png`.
Хвост 53б по Android закрыт; за телефоном — только хаптика.
