...
- [ ] **Google Play ОТЛОЖЕН — задача 66** (решение Артёма 28.08): страница «Подписки» закрыта до
      платёжного профиля продавца; Беларусь продавцом можно, Грузия — нет, а выплаты только на
      счёт в стране профиля (справка Google, исключение — SEPA). Белорусского счёта у Артёма нет,
      банк — Bank of Georgia → форму профиля НЕ отправляли (страна и тип необратимы). Запуск:
      покупки на iOS; Android — пейвол «пока нельзя» (62) без ключа, закрытый тест идёт.
...
- [x] **6в iOS — ПРОЙДЕНО 28.08 (Артём, iPhone, Sandbox-тестер, сборка `preview` `a320c6dd`):**
      ...
      Не прогнаны (ждут Артёма): модуль 3 и «Ещё N» при активном праве, «Управлять подпиской» →
      отмена → «Действует до …», авиарежим. ⚠️ Раздел «Sandbox-аккаунт» в Настройках → App Store
