...
5. **Язык — снимок при первой установке** (в `onRehydrateStorage`, там же, где назначается
   ...
   по всему коду и не про v1. ⚠️ **Пересмотрено волной фиксов финального ревью:** `restoreBackup`
   язык из файла ПРИМЕНЯЕТ, а не игнорирует — язык такая же восстанавливаемая настройка, как
   тема: человек переезжает на новый телефон, первая установка определяет язык устройства,
   а бэкап обязан вернуть ЕГО СОБСТВЕННЫЙ язык, иначе поверх русского дневника окажется
   английский интерфейс. Ограничитель — язык из файла применяется, только если он есть среди
   `AVAILABLE_LANGS` текущей сборки (`resolveImportedLang`, `src/lib/backup.ts`), иначе строка
   настроек показала бы язык без пункта в списке выбора. Первая версия этой спеки и logic-spec
   §7 писали «не трогает» вслед за старым комментарием в коде (он старше ветки, из задачи 11) —
   комментарий и спека ошибались, код уже применял язык.
...
- [x] `npx tsc --noEmit` чистый; `npm test` зелёный: новые сьюты `lang`, `i18nLangs`; оракул
      ...
      ✓ 719 тестов в 26 сьютах.
- [x] Веб-проверка 6а/6б: скриншоты настроек с открытым пикером в обеих темах и «Сегодня»/«Карты»
      после переключения на `es` — `docs/screenshots/27/` ✓ ПРОЙДЕНА;
      **лайв-проверка Артёма на iPhone пройдена 15.08 ✓, замечаний нет** (пикер, даты es/pt,
      колесо даты, DEV-строка детекции, переключение туда-обратно, пуш после смены языка).
      Ветка влита в main.
...
закрыты, каждая прошла отдельное ревью, плюс волна фиксов финального ревью.
Итог: **719 тестов в 26 сьютах** (было 574 в 23),
...
### Волна фиксов финального ревью (15.08)

Полный отчёт: `.superpowers/sdd/27-lang-infra/fix-wave-report.md`.

⚠️ **Главная находка — дыра между «файл валиден» и «файл применён».** `parseBackup` строго
проверяет, что `lang` в файле — один из четырёх языков (тест `backup.test.ts` это закрывал),
но ни один тест не проверял, что импортированный язык РЕАЛЬНО применяется к стору — а спека
и `logic-spec.md` §7 вдобавок утверждали обратное («восстановление язык не трогает»), хотя код
(старше ветки, из задачи 11) уже применял его безусловно. Решение изменено: `restoreBackup`
применяет язык файла — это восстанавливаемая настройка, как тема, — но с ограничителем по
`AVAILABLE_LANGS` текущей сборки, чтобы файл с недоступным языком не оставил строку настроек
без пункта в списке выбора. Ограничитель вынесен в чистую `resolveImportedLang` (`backup.ts`)
и закреплён тестами обоих случаев — того самого поведения, которое раньше не проверял никто.
⚠️ Заодно закрыты два пункта, которые отчёт задачи 27 сам же отложил как «не дефекты»:
устаревший doc-комментарий в `i18nPlurals.test.ts` (описывал прежнюю роль константы — убран)
и двойной вызов `deviceLocaleTags()` в одном выражении DEV-строки настроек (посчитан один раз).
Плюс контракт-тест на непустые `name.ru`/`name.en` у всех 78 карт (граница данных: пропади
английское название — заметит только пользователь es/pt, у которого имя карты падает
на английский фолбэк) и вынесение `AVAILABLE_LANGS` на чистую `availableLangs(dev, resources)` —
раньше решение «сборка отладочная ИЛИ есть строки» не проверялось ни одним тестом: в jest
`__DEV__` всегда истинный, и релизная ветка выражения не исполнялась ни разу.
...
- Окончания es/pt в стеммере поиска — появятся вместе с контентом (задача 28); там же —
  предупреждение про атомарную заливку name/keywords/search одного языка (найдено этой же
  волной фиксов, записано в backlog задачи 28).

Два пункта, отложенных здесь раньше — устаревший doc-комментарий в `i18nPlurals.test.ts`
и двойной вызов `deviceLocaleTags()` в DEV-строке настроек, — волна фиксов финального ревью
всё же закрыла: оба лежали рядом с настоящей находкой (дырой в языке бэкапа) и правились
той же правкой набело.
