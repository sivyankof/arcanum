...
7. **Цвет иконки замка.** Макет красит замок отдельным токеном `--lock` (#525a80 тёмная /
   #c9bd9c светлая), реализация берёт `muted` — по букве product-spec §2 и design-system §7;
   вместе с прозрачностью 0.55 узла результат близок к макетному. Записываем, чтобы следующая
   сверка не завела это как баг.
...
- [x] Таб «Курс»: 6 модулей одной лентой — шапка (overline «МОДУЛЬ N ИЗ 6», название, счётчики,
      pct%, замок на М3+) + тропа-змейка с пунктирной линией через узлы.
- [x] Ровно один золотой узел с пульсом и чипом «НАЧАТЬ УРОК» — первый непройденный сквозной;
      после DEV-«пройти урок» он переезжает на следующий узел, пройденный становится ✓.
- [x] Тап: current/done → `/lesson/[id]` («Урок готовится», название урока, шапка с «Курс»);
      locked → wobble + Warning-хаптика, без перехода (в вебе вибрации нет — не баг).
- [x] pct% шапки модуля: 0 / 17 (1 из 6) / 100 — считается из `lessonsProgress`;
      «· M КАРТ» скрыт у модулей без карт (М1, М6).
- [x] Автоскролл к текущему модулю при первом открытии таба; повторный тап по табу — к началу.
- [x] Обе темы читаемы; FadeUp-каскад на входе; reduce motion гасит пульс и боб.
- [x] `npx tsc --noEmit` чистый; `npm test` зелёный (новый сьют courseProgress).
- [x] Веб-сверка 6а: скриншоты 390×844 обеих тем в `docs/screenshots/07/`, сравнение с макетом
...
## Отчёт о реализации (12–13.08.2026)

Задача сделана двенадцатью шагами плана `docs/plans/07-course-path.md`, каждый — отдельным
коммитом с ревью. `npx tsc --noEmit` чистый, `npm test` — 166 тестов в 12 сьютах (было 146 в 11;
новый сьют `courseProgress` — 20 тестов).

### Что появилось

`src/lib/courseProgress.ts` — вся логика пути чистыми функциями без импортов react/expo:
состояния узлов (сквозное правило «current — первый непройденный на весь курс»), прогресс
модуля, счётчик карт, x-координаты змейки. `src/components/PathNode.tsx` (узел, три состояния,
пульс-кольцо, чип, качание замка), `CoursePath.tsx` (кривая Безье в px по замеренной ширине),
`ModuleHeader.tsx`. Экран `app/(tabs)/course.tsx` переписан лентой с автоскроллом к текущему
модулю; заглушка `app/lesson/[id].tsx`; DEV-строки прогресса в настройках; в сторе
`lessonsProgress` (persist version 4).

### Найдено веб-проверкой и исправлено

1. **Счётчик карт считал не то (найдено ДО старта, на предполётной сверке плана).** Формула
   «сумма длин `lesson.cards`» давала бы у М2 «16 КАРТ» при восьми разных картах: урок
   «Повторение» перечисляет все восемь заново. Эталон показывает «8 КАРТ». Перешли на подсчёт
   уникальных id; тестовая фикстура тоже была исправлена — она выдавала одинаковые id во всех
   уроках, и на ней подмена формулы была бы незаметна.
2. **Пульс кольца шёл вдвое быстрее задуманного.** Драйвер анимировался с ease-out и делился
   на 0.7 — формула молча предполагала линейность. Замер кривой: порог 70% достигался к 926мс
   вместо 1260мс, рост и гашение занимали 51% цикла вместо 70%, пауза выходила длиннее.
   Разбито на `withSequence`: 1260мс рост с ease-out + 540мс пауза — как keyframes эталона.
3. **Чип «НАЧАТЬ УРОК» ломался на две строки, подпись узла резалась вдвое.** Замер в браузере:
   чип 88×48 при родителе 76×76; у подписи инлайн-стиль `width: 132px`, а вычисленная ширина
   76px. Причина общая: оба элемента лежали прямо в контейнере узла 76×76, а react-native-web
   вдобавок ставит тексту `max-width: 100%`. Следствие — 20 из 32 названий уроков обрывались
   многоточием. ⚠️ Это расхождение веб/натив: на устройстве подпись была бы 132px, то есть
   веб-проверка показывала не то, что увидел бы Артём на телефоне. Починено одинаково для обеих
   платформ: чип и подпись переехали в собственные обёртки шире узла.
4. **Новое предупреждение в консоли.** Два новых файла передавали `pointerEvents` пропом, тогда
   как весь остальной проект задаёт его в стиле; react-native-web ругался на каждый узел.
   Перенесено в стиль — консоль по нашим компонентам чистая.

### Известные допуски (проверить на устройстве)

- **Чип и подпись соседнего узла соприкасаются**: подпись узла N занимает по вертикали 80..111
  от своего узла, чип узла N+1 — 53..85 при шаге 106. Замер на живом экране: пересечение
  прямоугольников 41×1px, визуально не проявляется — змейка разводит элементы по горизонтали.
  Шаг узлов НЕ увеличивали намеренно: чтобы пересечения не было геометрически при любых данных,
  нужен шаг ≥164px, а это растянуло бы тропу в полтора раза и увело пропорции от эталона
  (у эталона шаг/диаметр = 90/66 = 1.36, у нас 106/76 = 1.39). Если на iPhone метрики шрифта
  дадут заметное касание — правится одной константой `STEP` в `CoursePath.tsx`.
  ⚠️ Замер 41×1px сделан на первых узлах пути — это не худший случай: пересечение растёт,
  когда соседние узлы ближе всего по горизонтали, а именно на переходах x-паттерна змейки
  66%→42% (≈82px по горизонтали) и, в модулях длиннее шести уроков (циклится хвост паттерна),
  42%→24% (≈62px). Репро для проверки на устройстве: в DEV-строке настроек нажать «Пройти
  следующий урок» 9 раз — текущим станет `m2l6`, а над ним окажется двухстрочная подпись
  `m2l5` «Повторение: Дурак — Колесница»; для восьмиурочного модуля (М4) довести счёт до
  `m4l6`/`m4l7`.
- Оставшееся в консоли одно предупреждение `props.pointerEvents is deprecated` приходит из
  библиотеки: оно есть и на табе «Сегодня», где компонентов задачи 07 нет.
- Веб не показывает подпись «Курс» у кнопки «назад» на экране урока — `headerBackTitle`
  существует только на iOS; на устройстве подпись будет.
- Хаптика (качание замка, возврат с урока) в вебе не воспроизводится — проверяется только на 6в.

### Скриншоты

`docs/screenshots/07/` — старт пути и заглушка урока (тёмная), состояние с прогрессом и
переходом через границу модулей, обе темы до и после починки чипа и подписи.
