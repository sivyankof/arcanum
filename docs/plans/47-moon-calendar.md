...
- Значения из спеки/макета: `.date` 9.5/ls3.5 muted; `.h2` Cormorant 28 head, отступ 3; `.moonrow`
  ...
  12 muted, gap 4; `HALF_WINDOW = 0.92`; допуск тестов эфемерид ±10 мин; ΔT = 69 с.
...
describe('phaseInstant / moonEvents — точные моменты против эфемерид', () => {
  it.each(EPHEMERIS)('%s %s (%s) — в пределах ±10 мин', (kind, iso) => {
...
допуск ±10 мин). Средняя луна давала до ±14 ч к реальному моменту — для строки фазы терпимо,
