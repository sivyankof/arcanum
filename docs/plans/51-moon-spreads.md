...
лунные идут последними).

⚠️ **Формат файла сохранять ровно как есть: отступ 2 пробела и переводы строк CRLF.**
`spreads.json` отличается от `cards.json` (там отступ 1 и LF) — замерено побайтово 21.08.
Правку вносить руками (Edit), а не переписывать файл через `json.dump`: любой пересбор
переформатирует все 200+ строк, и настоящая правка утонет в диффе чистой перестановки —
тот же класс, что перестановка ключа `wordsStatus` в задаче 28а и CRLF в `set_status.py` (28к).
