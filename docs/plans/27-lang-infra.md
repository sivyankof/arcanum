...
```ts
      ...
      // есть). Дефолт `lang: 'ru'` в PERSIST_DEFAULTS —
      // только доливка старого файла без поля, настоящий первый язык назначается тут.
      // ⚠️ ОТМЕНЕНО волной фиксов финального ревью: здесь стояло «restoreBackup язык из файла
      // не трогает» — на деле применяет, с ограничителем по AVAILABLE_LANGS (см. спеку, решение 5).
      onRehydrateStorage: () => (state) => {
        if (state && state.installSeed === 0) {
          useApp.setState({
...
```
⚠️ Последняя фраза ОТМЕНЕНА волной фиксов финального ревью: `restoreBackup` язык ПРИМЕНЯЕТ
(`resolveImportedLang`, ограничитель по `AVAILABLE_LANGS`) — в logic-spec записано фактическое
поведение, а не это. Обоснование — решение 5 спеки.
